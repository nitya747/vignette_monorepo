...
function loadMemoryHistory(): ThumbnailHistoryItem[] {
  try {
...
function saveMemoryHistory(items: ThumbnailHistoryItem[]) {
  try {
...
const memoryHistory: ThumbnailHistoryItem[] = loadMemoryHistory();
...
export async function saveGeneration(payload: SaveHistoryPayload): Promise<ThumbnailHistoryItem> {
  ...
  const mockItem: ThumbnailHistoryItem = {
    id: generatedId,
...
export async function getUserHistory(
  ...
  let dbItems: ThumbnailHistoryItem[] = [];
  ...
  const itemMap = new Map<string, ThumbnailHistoryItem>();
