...
export function getMockCTRScore(title, niche, archetype, topic = '', keywords = '') {
    ...
    if (cleanSubject.toLowerCase().includes('thumbnail') || cleanSubject.toLowerCase().includes('prompt') || cleanSubject.toLowerCase().includes('draft')) {
        if (searchText.includes('jalebi'))
            cleanSubject = 'Jalebi';
        else if (searchText.includes('lofi') || searchText.includes('chill') || searchText.includes('sunset'))
            cleanSubject = 'Lofi Sunset Beats';
        else if (searchText.includes('ski') || searchText.includes('snow') || searchText.includes('mountain'))
            cleanSubject = 'Extreme Skiing';
        else if (searchText.includes('cook') || searchText.includes('kitchen') || searchText.includes('chef') || searchText.includes('food'))
            cleanSubject = 'Cooking Recipes';
        else if (searchText.includes('invest') || searchText.includes('stock') || searchText.includes('finance'))
            cleanSubject = 'Stock Investing';
        else if (searchText.includes('game') || searchText.includes('gaming') || searchText.includes('retro'))
            cleanSubject = 'Retro Gaming Setup';
        else if (searchText.includes('fit') || searchText.includes('gym') || searchText.includes('workout'))
            cleanSubject = 'Fitness Challenge';
        else
            cleanSubject = 'Creative Content';
    }
