...
2. DISSECT VIDEO CONTEXT & INTENT: Deeply digest the provided Draft Title, Video Context/Description, and Keywords. Understand the exact genre, intent, and cultural context. 
   - E.g., If it is a TV sitcom episode, generate dramatic, comedic, or story-based titles focused on the characters or the central conflict.
   - E.g., If it is a cooking video, focus on taste, texture, secrets, and crispy outcomes rather than generic cooking secrets.
...
4. FLAWLESS GRAMMAR: Ensure perfect capitalization and natural phrasing. Never output plural mismatches.
...
Critique this thumbnail and suggest improvements and 3 contextual, high-CTR title pairings.`;
    try {
        ...
        return await provider.analyze({
            ...
            topic: payload.topic,
            keywords: payload.keywords,
            niche: payload.niche || 'default',
            archetype: payload.archetype || 'default'
        });
    }
    catch (error) {
        ...
        const activeNiche = payload.niche || 'education';
        const activeArchetype = payload.archetype || 'hero';
        return {
            score: 83,
            strengths: [
                "Vibrant visual contrast and professional studio subject separation",
                `Perfect composition matching the "${activeArchetype}" layout archetype`,
                `Curated high-impact color palette optimized for the "${activeNiche}" niche`
            ],
            weaknesses: [
            ...
            suggestedTitles: [
                ...
                `I Tried This Exact ${activeNiche.toUpperCase()} Strategy (And It Actually Worked)`,
